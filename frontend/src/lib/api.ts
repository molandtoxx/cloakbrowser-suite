...
async function request<T>(
  ...
  const { timeout: _timeout, ...fetchOpts } = options ?? {};
  try {
...
export const api = {
  ...
  pasteClipboard: (id: string, text: string) =>
    request<{ ok: boolean }>(`/api/profiles/${id}/clipboard/paste`, {
      method: "POST",
      body: JSON.stringify({ text }),
    }),

  screenshot: (id: string): Promise<Blob> =>
    fetch(`/api/profiles/${id}/screenshot`).then((r) => {
